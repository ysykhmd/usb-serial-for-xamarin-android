...
namespace Aid.UsbSerial
{
	...
	{
        public const int CdcAcmNumberOfPorts = 1;

		public static readonly UsbSerialDeviceInfo CdcAcm = new UsbSerialDeviceInfo (typeof(CdcAcmSerialPort), CdcAcmNumberOfPorts);

		public Type DriverType { get; private set; }
		public int NumberOfPorts { get; private set; }

		public UsbSerialDeviceInfo(Type driverType, int numOfPorts)
		{
			if (driverType == null) throw new ArgumentNullException();
			if (!driverType.IsSubclassOf(typeof(UsbSerialPort))) throw new ArgumentException();
			if (numOfPorts < 1) throw new ArgumentException();

			DriverType = driverType;
			NumberOfPorts = numOfPorts;
		}
	}
}